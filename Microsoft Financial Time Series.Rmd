...
```{r}
# Specify the start and end dates for the estimation and forecast periods
start_estimation <- ymd("2019-04-25")  # Start date of the estimation period
end_estimation <- ymd("2024-04-18")    # End date of the estimation period
start_forecast <- ymd("2024-04-19")    # Start date of the forecast period
end_forecast <- ymd("2024-05-05")      # End date of the forecast period
...
# Forecast log returns using the selected method (Microsoft)
# Insert your forecasting code here for Microsoft

# For example, using auto.arima for forecasting
library(forecast)
forecast_model_msft <- auto.arima(estimation_sample_msft)
forecast_results_msft <- forecast(forecast_model_msft, h = length(forecast_sample_msft))
...
# Forecast log returns using the selected method (Visa)
# Insert your forecasting code here for Visa

# For example, using auto.arima for forecasting
forecast_model_visa <- auto.arima(estimation_sample_visa)
forecast_results_visa <- forecast(forecast_model_visa, h = length(forecast_sample_visa))
