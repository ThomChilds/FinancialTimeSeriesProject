  ...
  if (formulas){
    ...
    for (lag in lags){
      ...
      paste0("$\\text{Quantile}_\\text{",distr1,"}(",alpha,") = ",q,"$",
             "$\\hat{\\mu_{T+",lag,"|T}} = ",m,
             ", \\hat{\\sigma_{T+",lag,"|T}} = ",s, "$\n"),
      
      #VaR log returns
      paste0("$VaR(\\log \\text{returns})_{",alpha*100,"\\%, T + ",lag,"} = ",m," + ",s,"\\times",q,
             " = ", round(var_log_ret, 4) ,"$\n"),
      
      #VaR Euro
      paste0("$VaR_{",alpha*100,"\\%, T + ,",lag,",} = \\text{ Value in Euros } \\times VaR(\\log returns) = ",
...
enum_str_list = function(string_list, sep = ", ", last = " and "){
  ...
  # "last": seperator for the last element
    return(
      paste0(
        c(
          paste0(string_list[1:(length(string_list)-1)], collapse = ", "),
...
```{r start latex doc}
text_i = "\\subsection{A - VaR calculations}
...
cat(c("\\section{Value at Risk (VaR) Analysis}
\\label{sec:q2_value_at_risk_analysis}
", text_i), file = formulas_file, sep = "\n") 
```
...
text_i_msft = paste0("\n\nFollowing the modeling assumptions of this subtask, the available stock data and the given sum invested in the MSFT stock, the loss incuring over one day will be smaller than ",enum_str_list(msft_a_i_max_1_day_neg_change)," with " ,paste0(100*(1-alphas), "%"), "Confidence.


The due to simplicity of the model, the 5-day VaR is the same as the 1-day VaR.
")
cat(c(string_list, text_i_msft), file = formulas_file, append = TRUE, sep = "\n")
```
...
text_i_msft = paste0("\n\nFollowing the modeling assumptions of this subtask, the available stock data and the given sum invested in the V stock, the loss incuring over one day will be smaller than ",enum_str_list(msft_a_i_max_1_day_neg_change)," with " ,paste0(100*(1-alphas), "%"), "Confidence.
...
#write conclusion
text_ii_msft = paste0("\n\nFollowing the modeling assumptions of this subtask, the available stock data and the given sum invested in the MSFT stock, the loss incuring over one day will be smaller than ",
enum_str_list(
  format(
    msft_a_ii_table[1,((ncol(msft_a_ii_table)-2):(ncol(msft_a_ii_table)))],
         nsmall = 2, scientific = F
         )
  )

,"Euros with " ,paste0(100*(1-alphas), "%"), "Confidence.
The loss after 5 days will be smaller than", enum_str_list(
  format(
    msft_a_ii_table[5,((ncol(msft_a_ii_table)-2):(ncol(msft_a_ii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
cat(text_ii_msft, file = formulas_file, append = TRUE)
...
#write conclusion
text_ii_v = paste0("\n\nFollowing the modeling assumptions of this subtask, the available stock data and the given sum invested in the V stock, the loss incuring over one day will be smaller than ",
enum_str_list(
  format(
    v_a_ii_table[1,((ncol(v_a_ii_table)-2):(ncol(v_a_ii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
The loss after 5 days will be smaller than", enum_str_list(
  format(
    v_a_ii_table[5,((ncol(v_a_ii_table)-2):(ncol(v_a_ii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
text_ii_final = "Based on this model, the VaR is smaller for investments into the V stock than for investiments into the MSFT stock.
...
cat(c(text_ii_v,text_ii_final), file = formulas_file, append = TRUE, sep ="\n")
...
#write to file
text_iii_msft = paste0("\n\nFollowing the modeling assumptions of this subtask, the available stock data and the given sum invested in the MSFT stock, the loss incuring over one day will be smaller than ",
enum_str_list(
  format(
    msft_a_iii_table[1,((ncol(msft_a_iii_table)-2):(ncol(msft_a_iii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
The loss after 5 days will be smaller than", enum_str_list(
  format(
    msft_a_iii_table[5,((ncol(msft_a_iii_table)-2):(ncol(msft_a_iii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
cat(text_iii_msft, file = formulas_file, append = TRUE)
...
enum_str_list(
  format(
    v_a_iii_table[1,((ncol(v_a_iii_table)-2):(ncol(v_a_iii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
The loss after 5 days will be smaller than", enum_str_list(
  format(
    v_a_iii_table[5,((ncol(v_a_iii_table)-2):(ncol(v_a_iii_table)))],
         nsmall = 2, scientific = F
         )
  )
...
cat(c(text_iii_v,text_iii_final), file = formulas_file, append = TRUE, sep = "\n")
```
...
enum_str_list(
  format(
    msft_a_iv_table[1,((ncol(msft_a_iv_table)-2):(ncol(msft_a_iv_table)))],
         nsmall = 2, scientific = F
         )
  )
...
The loss after 5 days will be smaller than", enum_str_list(
  format(
    msft_a_iv_table[5,((ncol(msft_a_iv_table)-2):(ncol(msft_a_iv_table)))],
         nsmall = 2, scientific = F
         )
  )
...
cat(text_iv_msft, file = formulas_file, append = TRUE)
```
...
enum_str_list(
  format(
    v_a_iv_table[1,((ncol(v_a_iv_table)-2):(ncol(v_a_iv_table)))],
         nsmall = 2, scientific = F
         )
  )
...
The loss after 5 days will be smaller than", enum_str_list(
  format(
    v_a_iv_table[5,((ncol(v_a_iv_table)-2):(ncol(v_a_iv_table)))],
         nsmall = 2, scientific = F
         )
  )
...
cat(c(text_iv_v,text_iv_final), file = formulas_file, append = TRUE, sep = "\n")
```
...
```{r v model}
#specify model
# a_v_spec <-  rugarch::ugarchspec(variance.model=list(model="tGARCH", 
#                                                  garchOrder=c(1,2)),
#                         mean.model=list(armaOrder=c(1,0)),
#                              distribution.model="std")
#                               
#TODO secify different model
```
...
#get VaR
# msft_a_v_table = getVaR_table(a_v_spec, msft_5$log_rtn)
```
...
#get VaR
# v_a_v_table = getVaR_table(a_v_spec, v_5$log_rtn)
```
