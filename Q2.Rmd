...
print(paste("quantiles:", paste(round(qnorm(alphas),2),collapse = ",")))
...
getVaR_table = function(spec1, data1, lags = c(1,5), value_stock1 = value_stock, alphas1 = alphas, verbose = T){
  ...
  if (distr1 == "norm"){
    ...
    quant_fun = function(ps){
      return(qt(p = ps, df = fit1@fit$matcoef["shape"," Estimate"]))
    }
  } else {
  ...
  if (verbose){
    ...
    for (i in 1:nrow(max_neg_change_log_rtn)) {
      print(paste(rownames(max_neg_change_log_rtn)[i],"(log rtn)",
                  paste(round(max_neg_change_log_rtn,4)[i, ], collapse = " ")))
    }  
    
    string_list = c()
    
    for (j in lags){
      for (i in seq_along(alphas1)){
  
      var_log_ret = mu1[j] + sig1[j]*quant_fun(alphas1[i])
      q = round(quant_fun(alphas1[i]))
      m = round(mu1[j],4)
      s = round(sig1[j],4)
      string_list = c(string_list, "\n",
      paste0("$\\text{Quantile}_\\text{",distr1,"}(",alphas1[i],") = ",q,"$"),
      paste0("$\\hat{\\mu_{T+",j,"|T}} = ",m,", \\hat{\\sigma_{T+",j,"|T}} = ",s, "$"),
      paste0("$VaR(\\log \text{returns}) = ",m," + ",s,"\\times",q,
             " = ", round(var_log_ret, 4) ,"$"),
      paste0("$VaR_{5\\%} = \\text{ Value in Euros } \\times VaR(\\log returns) = 10.000.000 \\texteuro \\times",
    ...
    print(paste("                      ", paste(names(max_neg_change_euro), collapse = "     ")))
    for (i in 1:nrow(max_neg_change_euro)) {
      print(paste(rownames(max_neg_change_euro)[i],"(Euros)",
                  paste(round(max_neg_change_euro,2)[i, ], collapse = " ")))
    } 
  }
